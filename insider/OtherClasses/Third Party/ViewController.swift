...
let CMYKHalftoneFilter = CIFilter(name: "CIPhotoEffectFade")

let ComicEffect = "Comic Effect"
let ComicEffectFilter = CIFilter(name: "CIPhotoEffectChrome")

let Crystallize = "Crystallize"
let CrystallizeFilter = CIFilter(name: "CIPhotoEffectTransfer")

let Edges = "Edges"
let EdgesEffectFilter = CIFilter(name: "CIPhotoEffectInstant")

let HexagonalPixellate = "Hex Pixellate"
let HexagonalPixellateFilter = CIFilter(name: "CIPhotoEffectMono")

//let Invert = "Invert"
//let InvertFilter = CIFilter(name: "CIPhotoEffectNoir")

let Pointillize = "Pointillize"
let PointillizeFilter = CIFilter(name: "CIPhotoEffectProcess")

let LineOverlay = "Line Overlay"
let LineOverlayFilter = CIFilter(name: "CIPhotoEffectTonal")
...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        self.mainGroup.frame = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.width)
        self.imageView.frame = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.width)
        self.filtersControl.frame = CGRect(x: 0, y: 400, width: UIScreen.main.bounds.width, height: 60)
        ...
        filtersControl.selectedSegmentIndex = index
...
/*
 ...
 let iCMYKHalftone = "CMYK Halftone"
 let iCMYKHalftoneFilter = CIFilter(name: "CIPhotoEffectFade")
 
 let iComicEffect = "Comic Effect"
 let iComicEffectFilter = CIFilter(name: "CIPhotoEffectChrome")
 
 let iCrystallize = "Crystallize"
 let iCrystallizeFilter = CIFilter(name: "CIPhotoEffectTransfer")
 
 let iEdges = "Edges"
 let iEdgesEffectFilter = CIFilter(name: "CIPhotoEffectInstant")
 
 let iHexagonalPixellate = "Hex Pixellate"
 let iHexagonalPixellateFilter = CIFilter(name: "CIPhotoEffectMono")
 
 
 let Filter = [
 iCMYKHalftone: iCMYKHalftoneFilter,
 iComicEffect: iComicEffectFilter,
 iCrystallize: iCrystallizeFilter,
 iEdges: iEdgesEffectFilter,
 iHexagonalPixellate: iHexagonalPixellateFilter,
 
 ]
 
 let FilterName = [String](Filter.keys).sorted()
 ...
 case configurationFailed
	...
	public var videoQuality : VideoQuality       = .resolution352x288
 ...
 ///
 //self.mainGroup.frame = previewLayer.frame
 self.imageView.frame = previewLayer.frame
...
// movieFileOutputConnection?.videoOrientation = self.getVideoOrientation()
 ...
 let fileUrl = NSURL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("capturedvideo.MP4")
 
 self.lastPath = fileUrl!.path
 self.videoWriter = try? AVAssetWriter(outputURL: fileUrl!, fileType: AVFileTypeQuickTimeMovie)
 
 
 
 let outputSettings = [AVVideoCodecKey : AVVideoCodecH264, AVVideoWidthKey : NSNumber(value: Float(self.imageView.bounds.width)), AVVideoHeightKey : NSNumber(value: Float(self.imageView.bounds.height))] as [String : Any]
 
 self.writerInput = AVAssetWriterInput(mediaType: AVMediaTypeVideo, outputSettings: outputSettings)
 self.writerInput.expectsMediaDataInRealTime = true
 //self.audioWriterInput = AVAssetWriterInput(mediaType: AVMediaTypeAudio, outputSettings:outputSettings)
 
 self.videoWriter.add(self.writerInput)
 //self.videoWriter.add(self.audioWriterInput)
 
 self.adapter = AVAssetWriterInputPixelBufferAdaptor(assetWriterInput: self.writerInput, sourcePixelBufferAttributes: outputSettings)
 ...
 self.videoWriter.startSession(atSourceTime: self.starTime)
 ...
 //self.audioWriterInput.markAsFinished()
 print(self.lastPath)
 UIVideoAtPathIsCompatibleWithSavedPhotosAlbum(self.lastPath)
 self.videoWriter.finishWriting { () -> Void in
 print("FINISHED!!!!!")
 
 _ = UISaveVideoAtPathToSavedPhotosAlbum(self.lastPath, self, #selector(self.video), nil)
 //UISaveVideoAtPathToSavedPhotosAlbum(self.lastPath, self, "video:didFinishSavingWithError:contextInfo:", nil)
 }
